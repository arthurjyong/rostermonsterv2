...
- **Whole-run failure**: the strategy-specific aggregation rule for `UnsatisfiedResult` (§10.2). **Both first-release strategies run K independent seeded attempts** — each attempt is a full Phase 1 + Phase 2 composite under a derived candidate seed (e.g., per `python/rostermonster/solver/solver.py` lines 146–153, `SEEDED_RANDOM_BLIND` already loops `for index in range(maxCandidates)` with `candidate_seed = _per_candidate_seed(run_rng)` per iteration). The strategies differ in how they aggregate per-attempt failures:
