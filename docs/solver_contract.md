...
- **Whole-run failure**: the strategy-specific aggregation rule for `UnsatisfiedResult` (§10.2):
  - For **strategies that emit candidates from a single feasibility outcome** (`SEEDED_RANDOM_BLIND` per §12 — Phase 1 + Phase 2 composite produces one rule-engine-feasibility outcome under the active `seed`; the K candidates emitted up to `maxCandidates` come from that single feasible fill via tie-break diversifications, NOT from K independent attempts at re-feasibilization), whole-run failure occurs whenever Phase 2 cannot complete the fill. ANY unfillable demand unit triggers `UnsatisfiedResult`; partial-success across K candidate variations is NOT a recovery path because there is only one feasibility outcome to begin with. This preserves the original M2 C1 closure semantics ("if the active strategy cannot fill every non-fixed demand unit ... return `UnsatisfiedResult`"; pre-M6 SEEDED_RANDOM_BLIND-only behavior).
  - For **multi-trajectory strategies that run K independent re-feasibilization attempts** (`LAHC` per §12A.2, where each of the K trajectories independently invokes `SEEDED_RANDOM_BLIND` Phase 1 + Phase 2 with its own `trajectorySeed_i` — different seeds produce different exploration paths and different feasibility outcomes), whole-run failure occurs **only when ALL K trajectories' seed-construction attempts fail** per §12A.8. If at least one trajectory produces a fully-valid rule-engine-valid candidate, the strategy emits a non-empty `CandidateSet` containing the successful trajectories' emitted rosters per §12A.1 step 5 (`bestRoster` per trajectory, NOT terminal `currentRoster`) — `K' = K - dropped_count >= 1` candidates per §12A.8; the run is NOT a whole-run failure even if some trajectories dropped.
