...
`LAHC` is a score-aware local-search strategy that escapes local optima via a history-list accept criterion. It is the second registered strategy alongside §12's `SEEDED_RANDOM_BLIND`. LAHC conforms to the §11 strategy-interface contract and activates the §11.2 `scoringConsultation: "READ_ONLY_ORACLE"` extension clause. (LAHC's strategy registration alone via §11.2 would not have required a `contractVersion` bump — the strategy-interface itself is unchanged, additive only — but M6 C1 closure also promoted `strategyId` to a required §9 input, which IS a §2-triggering input-shape change; net solver contract bumped `1` → `2`. See §2 + D-0067 sub-decision 7.)

### 12A.1 Algorithm
Per LAHC trajectory, state variables are: `currentRoster`, `currentScore`, `historyList[L]`, `bestRoster`, `bestSoFar`, `idleIters`, `currentIter`. Steps:
1. **Seed roster.** Produce an initial valid roster via `SEEDED_RANDOM_BLIND`'s two-phase composite (§12 Phase 1 CR seeding + §12 Phase 2 most-constrained-first fill). Initialize `currentRoster := seedRoster`, `currentScore := score(seedRoster)` (via the read-only oracle, §12A.6), `bestRoster := seedRoster`, `bestSoFar := currentScore`, `idleIters := 0`, `currentIter := 0`.
...
3. **Inner loop.** Iterate per pass:
   ...
   - **c. Accept criterion.** Accept iff `proposedScore >= currentScore` OR `proposedScore >= historyList[currentIter mod L]`. The first clause accepts strict improvements over the current state immediately (greedy hill-climbing arm — never rejects a genuine improvement, even when the history-list slot still carries an older high score). The second clause accepts non-improving moves whose score meets the aged-out history-list threshold (the late-acceptance arm that escapes local optima by allowing temporary worsening). This is the standard LAHC accept rule per Burke & Bykov 2008/2017 (originally formulated for cost minimization with `newCost <= currentCost OR newCost <= historyList[v]`; adapted here for `HIGHER_IS_BETTER` per `docs/scorer_contract.md` §10). A single-clause rule using only the history-list arm would block strict improvements when `historyList[currentIter mod L]` still carries a stale high from before a recent worsening — defeating the hill-climbing component of the heuristic.
   ...
   - **f. Best-roster + idle counter.** If `currentScore > bestSoFar`, set `bestSoFar := currentScore`, `bestRoster := currentRoster`, and reset `idleIters := 0`; else `idleIters += 1`. (`bestSoFar` advances only on strict improvement so `idleIters` stays monotone in no-improvement iterations; `bestRoster` is the corresponding roster snapshot — both update atomically with `bestSoFar`.)
   - **g. Increment.** `currentIter += 1`.
4. **Inner loop termination.** Stop when EITHER `idleIters >= idleThreshold` OR `currentIter >= maxIters` (§12A.3).
5. **Emit.** The trajectory's `bestRoster` (the roster selected by maximum `bestSoFar` over the trajectory) is emitted as a single `TrialCandidate`. **NOT the terminal `currentRoster`** — LAHC's late-acceptance criterion (§12A.1.c) routinely accepts moves below `bestSoFar` after first reaching it, so the terminal `currentRoster` can be strictly worse than a roster the trajectory already discovered. Under the scorer's `HIGHER_IS_BETTER` direction (`docs/scorer_contract.md` §10), emitting the terminal would discard a better candidate the trajectory already found and would surface as analyzer / writeback regression. **The emitted `TrialCandidate`'s score-summary field (`docs/domain_model.md` §12.3) MUST stay unpopulated per §10.1** — `bestSoFar` is internal LAHC state used to choose `bestRoster`, NOT a score the solver writes back into the `TrialCandidate`; downstream scoring per `docs/scorer_contract.md` populates the score field on the emitted candidate as it does for `SEEDED_RANDOM_BLIND` outputs. `bestSoFar` per trajectory surfaces only in `SearchDiagnostics` per §12A.9. `bestRoster` is rule-engine-valid by construction (the move generator filters invalid moves per §12A.1.a step iii; `bestRoster` was once a `currentRoster` after a successful state update).
...
- `maxIters = 100,000`.
- `swapProbability = 0.5` (primary-move-type bias; added M6 C4 PR-A — default reproduces historical hardcoded `rng.random() < 0.5` coin-flip byte-identically; tunable in `[0.0, 1.0]` per §12A.7).

Rationale: `L = 1000` is a literature default for combinatorial problems at roster scale (Burke & Bykov 2017's LAHC study across NRP-like domains used `L ∈ [500, 10000]`). `idleThreshold = 5L` is a common LAHC convention — gives the algorithm a chance to escape multiple local optima before declaring convergence. `maxIters = 100k` is the documented v1 ceiling per trajectory; actual per-trajectory wall-clock runtime depends on per-iteration scoring cost. `swapProbability = 0.5` is the historically-hardcoded coin-flip bias, preserved as a default so behavior pre-/post-M6-C4 is byte-identical.

**Empirical post-M6-closure note** (per `docs/decision_log.md` D-0069 + `docs/future_work.md` FW-0036 / FW-0037): the documented `L=1000` / `idleThreshold=5000` defaults sit past the cliff in M6 C4's dev-copy iter-budget regime — the empirically-tuned sweet-spot tuple on the May 2026 ICU/HD dev-copy fixture is `L=10` / `idleThreshold=2000` / `swapProbability=0.5` with `K=10–20`. These tuned values are NOT pinned to module defaults pending multi-fixture validation per FW-0036 / FW-0037; they are reachable via the §12A.7 CLI override surface for maintainer experimentation. Future revision of the §12A.5 defaults requires multi-fixture confirmation that the tuned tuple holds across cycles.

**Cloud sync-budget caveat**: K trajectories run sequentially per §12A.2. With cloud-mode default `maxCandidates = 32` (per `docs/cloud_compute_contract.md` §9.3) and `maxIters = 100k` per trajectory, the worst-case sequential runtime can exceed Cloud Run's `UrlFetchApp` 6-minute synchronous budget (per `docs/decision_log.md` D-0051). Cloud-specific LAHC defaults are NOT pinned by M6 — D-0068 narrowed M6 C3 to local-first and carved cloud-mode LAHC integration off to **FW-0035**, which is parked through M6 closure (D-0069). The cloud HTTP wrapper (`python/rostermonster_service/app.py`) ships `SEEDED_RANDOM_BLIND` only in v1; cloud-mode LAHC defaults will be pinned at FW-0035 promotion (likely co-promoting with FW-0036 K-default work and FW-0038 parallel-solver / M7 work, since cloud LAHC's K + L defaults need to be sync-budget-aware AND parallelism-aware). The defaults specified here (`L=1000`, `idleThreshold=5000`, `maxIters=100k`, `swapProbability=0.5`) are the LAHC algorithm's v1 ceiling; defaults are tunable via the maintainer-only surface per §12A.7.
...
- If a LAHC trajectory's inner loop terminates by `idleThreshold` or `maxIters` without strict improvement over the seed score (`bestSoFar == seedScore` at termination), the trajectory emits its `bestRoster` per §12A.1 step 5, which equals the seed roster in this case (since `bestSoFar` never strictly improved past seed score, `bestRoster` was never updated past `seedRoster`). The terminal `currentRoster` may have wandered away from `seedRoster` via late-acceptance moves, but `bestRoster` (== `seedRoster` here) is what's emitted. `bestRoster` is rule-engine-valid by construction (the move generator filters rule-engine-invalid moves per §12A.1.a step iii); emitted as a `TrialCandidate`.
...
- **Whole-run failure (`UnsatisfiedResult`)**: LAHC returns `UnsatisfiedResult` per §10.2 / §10.3 if and only if ALL `K` trajectories' seed roster steps fail (every `SEEDED_RANDOM_BLIND` invocation returns `UnsatisfiedResult` for `trajectorySeed_0..K-1`). The `unfilledDemand` and `reasons` fields MUST surface the **deterministic complete union** of the per-trajectory `SEEDED_RANDOM_BLIND` failure data per §10.2 — every `(dateKey, slotType, unitIndex)` and every `ValidationIssue` reported by ANY of the `K` failed trajectories MUST appear (de-duplication of byte-identical entries is permitted; ordering MUST be deterministic given seed + inputs). Implementations MUST NOT omit failures that occurred in any trajectory; surfacing only an "implementation-chosen subset" would leave operators and tests blind to causes that affected a strict subset of trajectories. If at least one trajectory succeeds, LAHC emits a non-empty `CandidateSet` containing the successful trajectories' emitted rosters per §12A.1 step 5 (`bestRoster` per trajectory, NOT terminal `currentRoster`) — `K' = K - dropped_count` candidates, where `K' >= 1`. Empty `CandidateSet` on the success branch is a contract-breaking defect per §10.1.
...
- `perTrajectoryBestScore[i]` — emitted-roster score per trajectory (the `bestSoFar` at termination, which is the score of the emitted `bestRoster` per §12A.1 step 5; post-§12A.4 determinism check); undefined for `SEED_FAILED`.
- `perTrajectoryTerminalScore[i]` — terminal `currentScore` per trajectory at inner-loop termination (informative only; may differ from `perTrajectoryBestScore[i]` when late-acceptance moves left `currentRoster` below `bestRoster` per §12A.1 step 5); undefined for `SEED_FAILED`.
...
Implementation location: a shared module accessible to both the local CLI and Cloud Run Service orchestrator. **Pinned at M7 C2 Task 2A (2026-05-10) to `python/rostermonster/solver/seeds.py`**, which owns both the public helper `derive_K_seeds` AND the underlying `_UINT64_MASK` constant + `_per_candidate_seed(rng)` primitive. `solver.py` no longer holds these primitives — the local-CLI K-trajectory loop calls `derive_K_seeds(seed, terminationBounds.maxCandidates)` once and iterates over the returned list. The helper is re-exported from `rostermonster.solver` (the package `__init__.py`) so the orchestrator imports it as `from rostermonster.solver import derive_K_seeds`.
...
- **Whole-run failure**: the strategy-specific aggregation rule for `UnsatisfiedResult` (§10.2). **Both first-release strategies run K independent seeded attempts** — each attempt is a full Phase 1 + Phase 2 composite under a derived candidate seed (e.g., `SEEDED_RANDOM_BLIND` loops over the K seeds returned by `derive_K_seeds(seed, maxCandidates)` per §12A.10). The strategies differ in how they aggregate per-attempt failures:
  - For **`SEEDED_RANDOM_BLIND`** (§12), whole-run failure occurs when **ANY attempt** cannot complete the fill. The strategy aborts on the first unfillable attempt and surfaces that attempt's `unfilledDemand` directly. This preserves the M2 C1 closure semantics + the existing Python implementation's any-failure-fails policy. M6 C2 Task 2A's search-strategy-abstraction extraction MUST preserve this byte-identical baseline behavior — refactoring SEEDED_RANDOM_BLIND must not silently shift it into LAHC's all-fail-fails aggregation.
  - For **`LAHC`** (§12A.2), whole-run failure occurs **only when ALL K trajectories' seed-construction attempts fail** per §12A.8. Per-trajectory drops are legitimate; if at least one trajectory produces a fully-valid rule-engine-valid candidate, the strategy emits a non-empty `CandidateSet` containing the successful trajectories' emitted rosters per §12A.1 step 5 (`bestRoster` per trajectory, NOT terminal `currentRoster`) — `K' = K - dropped_count >= 1` candidates per §12A.8; the run is NOT a whole-run failure even if some trajectories dropped.
...
- Consistent with `docs/decision_log.md` D-0066 (M6 framing) + D-0067 (M6 C1 LAHC algorithm spec): solver contract bumps to `contractVersion: 2` for the §9 input-shape change (`strategyId` promoted to declared required input); LAHC's strategy registration alone via §11.2's extension clause is additive and would not have required a bump on its own. Two strategy-aware fields cross the solver boundary: (i) **`strategyId` as a required input** in §9 — callers MUST pass it explicitly per the v2 boundary; and (ii) **`solverStrategy` as an envelope output** — the wrapper envelope records which strategy ran so the M5 analyzer + ops trail can compare runs (envelope-shape additive bump location settled in M6 C3).
