...
- **Checkpoints:** C1 Python analyzer engine + analysis contract draft (active; Phase 1 docs landed in PR #110 closing the deferred-design items per D-0056..D-0058 + FW-0031 + FW-0032; Phase 2 implementation pending in `python/rostermonster/analysis/`); subsequent checkpoints (Apps Script analyzer renderer, upload portal, live operator validation) outlined in §7 but not yet activated.
...
- **C1 — Python analyzer engine + analysis contract draft.** Standalone `analyze(snapshot, envelope, fullSidecar, *, topK, generatedAt, analysisConfig=None) → AnalyzerOutput` function in new `python/rostermonster/analysis/` module. Phase 1 docs (PR #110) drafted `docs/analysis_contract.md` (`contractVersion: 1`) defining input = full snapshot JSON + wrapper envelope + `candidates_full.json` (CSV sidecar explicitly excluded), output = single `AnalyzerOutput` JSON file, pure score-rank top-K with selector-cascade tiebreak (D-0056), Tiers 1–5 v1 emission scope (Tier 6 deferred to FW-0032; Tier 7 renderer-derived per D-0058), parser-overlay module reuse for `cumulativeCallPoints` to match scorer-equivalent weights, fail-loud snapshot↔envelope↔sidecar coherence at admission, solver-agnostic by contract (D-0056 + §12). Phase 2 implementation pending. **Active.**
...
M5 C1 follows the established Phase 1 docs → Phase 2 code → Phase 3 closure cadence (M2 / M3 / M4 precedent).

- **Phase 1 — analysis contract draft (this PR):** new contract `docs/analysis_contract.md` (`contractVersion: 1`); D-0056 (analyzer top-K = pure score-rank with selector-cascade tiebreak, no diversity heuristic); D-0057 (analyzer I/O shape = full snapshot + wrapper envelope + FULL sidecar JSON in, single `AnalyzerOutput` JSON out); D-0058 (Tiers 1–5 v1 emission scope, Tier 6 deferred to FW-0032, Tier 7 renderer-derived, snapshot-extension fields parked as FW-0031). Closes D-0055 sub-decisions 1, 4, 6, 7, 10's deferred concrete-shape items.
- **Phase 2 — analyzer engine implementation:** standalone `analyze(snapshot, envelope, fullSidecar, *, topK, generatedAt, analysisConfig=None) → AnalyzerOutput` in new `python/rostermonster/analysis/` module per `docs/analysis_contract.md` §6; CLI surface (planned `--analyze` subcommand or `python -m rostermonster.analysis`) accepting snapshot JSON + `final_envelope.json` + `candidates_full.json` paths and `--top-k N` (default 5, fail-loud out of `[1, 20]`); outputs single `AnalyzerOutput` JSON to stdout or `--output PATH`. Tier 1–5 aggregates per `docs/analysis_contract.md` §10 + §13 (Tier 6 deferred to FW-0032; Tier 7 renderer-derived). Test fixtures off the May 2026 ICU/HD dev-copy. Byte-identical determinism per §15.
- **Phase 3 — closure:** M5 C1 closure PR; bump active checkpoint pointer to M5 C2; preserve M5 C1 closure entry in §11 with the one-line "what works now" summary.
...
- contracts settled across M2 + M3 + M4 + M5 C1 Phase 1: snapshot, snapshot_adapter, parser_normalizer, template_artifact, sheet_generation, rule_engine, scorer (v3), solver, selector (v2), writeback (`contractVersion: 1`, §9 6 categories), cloud_compute (`contractVersion: 1`), analysis (`contractVersion: 1`, drafted in M5 C1 Phase 1 per D-0056 / D-0057 / D-0058 — analyzer engine input/output, pure score-rank top-K with selector-cascade tiebreak, solver-agnostic property, Tiers 1–5 v1 emission scope; Tier 6 deferred to FW-0032; Tier 7 renderer-derived).
