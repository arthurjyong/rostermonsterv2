...
- **Phase 1 — analysis contract draft (this PR):** new contract `docs/analysis_contract.md` (`contractVersion: 1`); D-0056 (analyzer top-K = pure score-rank, no diversity heuristic); D-0057 (analyzer I/O shape = wrapper envelope + FULL sidecar JSON in, single `AnalyzerOutput` JSON out); D-0058 (Tiers 1–6 emission scope, Tier 7 renderer-derived, snapshot-extension fields parked as FW-0031). Closes D-0055 sub-decisions 1, 4, 6, 7, 10's deferred concrete-shape items.
- **Phase 2 — analyzer engine implementation:** standalone `analyze(envelope, fullSidecar, *, topK) → AnalyzerOutput` in new `python/rostermonster/analysis/` module per `docs/analysis_contract.md` §6; CLI surface (planned `--analyze` subcommand or `python -m rostermonster.analysis`) accepting `final_envelope.json` + `candidates_full.json` paths and `--top-k N` (default 5, fail-loud out of `[1, 20]`); outputs single `AnalyzerOutput` JSON to stdout or `--output PATH`. Tier 1–6 aggregates per `docs/analysis_contract.md` §10 + §13. Test fixtures off the May 2026 ICU/HD dev-copy. Byte-identical determinism per §15.
...
- **2026-05-04 (M5 C1 Phase 1 docs):** Drafted new contract `docs/analysis_contract.md` (`contractVersion: 1`) closing D-0055's deferred concrete-shape items. D-0056 (analyzer top-K = pure score-rank, no diversity heuristic — operator-tunable `--top-k N`, default 5, bounds `[1, 20]`, fail-loud out of range; tiebreak by ascending `candidateId`; promotes Tier 5 `pairwiseHammingDistance` from optional flavor to load-bearing operator diagnostic). D-0057 (analyzer I/O shape — input = wrapper envelope + `candidates_full.json` only, CSV explicitly excluded; output = single `AnalyzerOutput` JSON file symmetric with D-0044/D-0045 wrapper-envelope pattern). D-0058 (Tiers 1–6 emission scope locked in v1; Tier 7 decision-support tags renderer-derived; snapshot-extension fields parked as FW-0031). FW-0031 (snapshot-extension analyzer fields — senior-junior pairing, leave-history-aware analysis, rotation-conflict surfacing — added with four explicit revisit triggers). No code changes in this PR — Phase 1 docs only, mirroring the M2/M3/M4 docs-first cadence; analyzer-engine implementation is M5 C1 Phase 2.
