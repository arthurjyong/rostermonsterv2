...
- closed-checkpoint trail for M2 C3 + M2 C2 + M2 C1 in §11 (sign-off notes included), anchored by D-0024..D-0036 across the rule-engine / scorer / solver / selector contracts, the `unitIndex` doctor-admissibility equivalence, the selector v1 → v2 bump, the parser/normalizer implementation, the dateKey adapter normalization rule, the doctor-name matching algorithm, the locator-direct provenance shape, and the late-M2 Apps Script extractor commitment
