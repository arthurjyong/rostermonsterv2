...
- **C4 — Minimal rule/scorer/solver integration.** *(active; see §8)* Implement the three remaining M2 compute-pipeline stages whose contracts settled in M2 C1 — rule engine (`docs/rule_engine_contract.md`), scorer (`docs/scorer_contract.md`), solver (`docs/solver_contract.md`) — against the now-shipped parser/normalizer's CONSUMABLE `NormalizedModel` output from M2 C3. Selector implementation is intentionally NOT in C4 — its retention modes and sidecar artifacts fit naturally into "local run artifact basics" (next §5 entry). Output of C4 = sorted `CandidateSet` with full `ScoreResult` per candidate, ready for selector consumption in a later checkpoint.
...
- **Active checkpoint:** `Minimal rule/scorer/solver integration` (M2 C4)

M2 C4 was activated on 2026-04-27 and is the current execution focus. It implements the three remaining M2 compute-pipeline stages whose contracts settled in M2 C1 (rule engine, scorer, solver) for ICU/HD first-release inputs, wired onto the parser/normalizer's CONSUMABLE `NormalizedModel` output from M2 C3 (which closed on the same day). The selector implementation is intentionally **NOT** in C4 — selector retention modes (`BEST_ONLY` / `FULL`) and sidecar artifacts (`candidates_summary.csv`, `candidates_full.json`) are output-layer concerns that fit naturally into the next M2 candidate "local run artifact basics" per §5. Stack ownership remains local-first Python per D-0018; output of this checkpoint is a sorted `CandidateSet` with full `ScoreResult` per candidate. Selector / writeback / Apps Script extractor implementations remain deferred per §10 + D-0036.

## 9. Task list for the current checkpoint

M2 C4 — `Minimal rule/scorer/solver integration` (active).

- **T1 — Rule engine implementation.** Implement all 5 first-release hard rules statelessly per `docs/rule_engine_contract.md` §11 (`BASELINE_ELIGIBILITY_FAIL`, `SAME_DAY_HARD_BLOCK`, `SAME_DAY_ALREADY_HELD`, `UNIT_ALREADY_FILLED`, `BACK_TO_BACK_CALL`); full-violation canonical ordering per §12; FixedAssignment scoped handling per §15 (rule engine MUST NOT re-adjudicate fixed assignments against themselves but MUST apply downstream-validity checks against fixed-assignment neighbors); statelessness per §13 (no caching exposed at contract surface); soft-effect separation per §16 (rule engine MUST NOT consult `prevDayCallSoftPenaltyTrigger` / `callPreferencePositive`); determinism per §17. Public surface `(normalizedModel, ruleState, proposedUnit) → Decision`. ICU/HD first-release inputs only. Tests cover each hard rule in isolation, full-list canonical ordering, FixedAssignment scoped admission, and the statelessness contract surface. *(Will be Done when commit lands.)*
- **T2 — Scorer implementation.** Implement all 9 first-release components per `docs/scorer_contract.md` + `docs/domain_model.md` §11.2 (`unfilledPenalty`, `pointBalanceWithinSection`, `pointBalanceGlobal`, `spacingPenalty`, `preLeavePenalty`, `crReward`, `dualEligibleIcuBonus`, `standbyAdjacencyPenalty`, `standbyCountFairnessPenalty`); pure-function shape `(allocation, normalizedModel, scoringConfig) → ScoreResult`; mandatory `HIGHER_IS_BETTER` direction-guard property test per §13; `crReward` diminishing-marginal-utility curve per §12 (strict-monotonic-decrease per doctor); scorer-owned soft-effect reading from `DailyEffectState` per §14; required component breakdown on every `ScoreResult` per §10 (every component appears even when contributing zero). Concrete weight values referenced from v1 per `docs/future_work.md` FW-0014 during implementation. *(Will be Done when commit lands.)*
- **T3 — Solver implementation.** Implement `SEEDED_RANDOM_BLIND` strategy per `docs/solver_contract.md` §12 (Phase 1 = `CR_MINIMUM_PER_DOCTOR` preference seeding, Phase 2 = `MOST_CONSTRAINED_FIRST` fill); `crFloor` `SMART_MEDIAN` (default) + `MANUAL` modes per §13, with computed `X` logged in `SearchDiagnostics` at run start per §13.4; whole-run failure on any unfillable slot per §14; `maxCandidates`-only termination per §15 (no wall-clock); byte-identical determinism under fixed `(seed, normalizedModel, ruleEngine, fillOrderPolicy, terminationBounds, preferenceSeeding)` inputs per §16; consumes the rule engine handle from T1; remains scoring-blind throughout (never reads scoring per §11.1). Tests cover Phase 1 / Phase 2 behavior, `crFloor` modes, whole-run failure path, and seeded determinism. *(Will be Done when commit lands.)*
- **T4 — Integration smoke test.** End-to-end run: ICU/HD May 2026 snapshot fixture (already committed under `python/tests/data/`) → parser/normalizer (already shipped) → solver + rule engine → scorer → sorted `CandidateSet` with full `ScoreResult` per candidate. Verify the pipeline produces a non-empty `CandidateSet` under realistic input, byte-identical output across re-runs under fixed seed, and zero rule-engine hard-rule violations across emitted candidates. *(Will be Done when validation lands.)*
- **T5 — Update `docs/delivery_plan.md`.** Closure transition matching the C3 T4 pattern: §7 mark C4 closed, §8 switch active-checkpoint state, §9 placeholder, §11 record M2 C4 sign-off, §12 change-log entries, §15 initial-seed paragraph refresh. *(Done with closure commit.)*

C4 closes when T1–T4 are Done and the branch PR(s) merge. T5 closes the delivery-plan transition. Sign-off note is recorded in §11 at that point.
...
This version reflects the 2026-04-27 activation of M2 C4 (`Minimal rule/scorer/solver integration`) on top of the 2026-04-27 closure of M2 C3 (`Parser/normalizer implementation closure`), the 2026-04-26 closure of M2 C2 (`Selector contract closure`), the 2026-04-25 closure of M2 C1 (`Rule engine + scorer + solver contract closure`), and the 2026-04-22 closure of M1.1 (`Operator-facing launcher`), and is currently in the following state:
- active milestone `Minimal local compute pipeline` (M2)
- active checkpoint `Minimal rule/scorer/solver integration` (M2 C4), implementing the three remaining M2 compute-pipeline stages whose contracts settled in M2 C1 (rule engine, scorer, solver) for ICU/HD first-release inputs against the parser/normalizer's CONSUMABLE `NormalizedModel` from C3. Selector implementation deferred to a later checkpoint paired with retention/output mechanics
- closed-checkpoint trail for M2 C3 + M2 C2 + M2 C1 in §11 (sign-off notes included), anchored by D-0024..D-0037 across the rule-engine / scorer / solver / selector contracts, the `unitIndex` doctor-admissibility equivalence, the selector v1 → v2 bump, the parser/normalizer implementation, the dateKey adapter normalization rule, the doctor-name matching algorithm, the locator-direct provenance shape, the late-M2 Apps Script extractor commitment, and the operator-tuneable scoring config surface (scorer v1 → v2 with `pointRules`)
- the parser/normalizer module is now functionally complete for ICU/HD first-release inputs end-to-end and lives at `python/rostermonster/`; 24/24 tests pass via standalone runners; the test-only xlsx → JSON bridge proves the inbound Apps Script ↔ Python seam works against real ICU/HD May 2026 volume, with the production Apps Script extractor pinned to a late-M2 checkpoint per D-0036
