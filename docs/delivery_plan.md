...
This version reflects the 2026-04-26 activation of M2 C3 (`Parser/normalizer implementation closure`) on top of the 2026-04-26 closure of M2 C2 (`Selector contract closure`), the 2026-04-25 closure of M2 C1 (`Rule engine + scorer + solver contract closure`), and the 2026-04-22 closure of M1.1 (`Operator-facing launcher`), and is currently in the following state:
- active milestone `Minimal local compute pipeline` (M2)
- active checkpoint `Parser/normalizer implementation closure` (M2 C3), the **first M2 checkpoint that ships executable code** — implementing the local-first Python parser/normalizer module per `docs/parser_normalizer_contract.md`, `docs/snapshot_contract.md`, and `docs/domain_model.md` for ICU/HD first-release inputs (downstream-stage implementation deferred per §10)
