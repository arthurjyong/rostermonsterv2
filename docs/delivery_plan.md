...
M5 C2 detailed scope (exact `analyzer.js` placement — central library vs. new Apps Script project; comparison-tab UX; how K roster tabs compose with writeback's roster tab on the same spreadsheet) is deferred to a dedicated design thread that opens after this closure PR merges. Thread output: a renderer-side design note (or a renderer contract / library docstring) pinning the C2 sub-PR scope. Implementation follows the same docs-first cadence M5 C1 used (contract / spec → code → closure), but without the formal "Phase N" labels — each sub-PR is described by what it accomplishes.
...
- **2026-05-05 (Apps Script directory rename, this PR):** Renamed three Apps Script directories from milestone-prefixed to role-based names per D-0059: `apps_script/m1_sheet_generator/` → `apps_script/launcher/`, `apps_script/m2_template_bound_script/` → `apps_script/bound_shim/`, `apps_script/m2_extractor_library/` → `apps_script/central_library/`. Cloud-side Apps Script project name `Roster Monster Extractor Library` is correspondingly renamed to `Roster Monster Central Library` (one-time maintainer step via Apps Script Project Settings — clasp Script ID stable, no clasp config / library dependency surface change). Cross-doc sweep updates `docs/decision_log.md`, `docs/cloud_compute_contract.md`, `docs/snapshot_adapter_contract.md`, `docs/writeback_contract.md`, `docs/future_work.md` FW-0029, README, AGENTS, library README files, Apps Script source comments, `.gitignore`, project memory `project_m2_c9_setup_ids.md` + `reference_clasp_run_oauth.md`. Supersedes the M2 C9 closure note's "milestone-prefix-by-introduction" convention. No code-behavior change; 225/225 Python tests still pass.
...
- contracts settled across M2 + M3 + M4 + M5 C1: snapshot, snapshot_adapter, parser_normalizer, template_artifact, sheet_generation, rule_engine, scorer (v3), solver, selector (v2), writeback (`contractVersion: 1`, §9 6 categories), cloud_compute (`contractVersion: 1`), analysis (`contractVersion: 1`, drafted under M5 C1 per D-0056 / D-0057 / D-0058 — analyzer engine input/output, pure score-rank top-K with selector-cascade tiebreak, solver-agnostic property, Tiers 1–5 v1 emission scope; Tier 6 deferred to FW-0032; Tier 7 renderer-derived).
