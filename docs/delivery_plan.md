...
- Subsequent M2 checkpoints remain candidate-ordered from §5 (minimal rule/scorer/solver integration; local run artifact basics; snapshot-extraction Apps Script implementation per D-0036) and will be scoped individually after C3 closes.

## 8. Current active checkpoint
- **Active checkpoint:** none currently active; next M2 checkpoint to be scoped separately following the same "no active checkpoint between two checkpoints" pattern that bridged M2 C2 closure to M2 C3 activation (2026-04-26 → 2026-04-26).

M2 C3 closed on 2026-04-27 with the parser/normalizer module shipping end-to-end against ICU/HD first-release inputs — admission pipeline (T1), normalizer-side provenance and §17 handoff defense (T2), and real-data hand-test against the May 2026 dev-copy (T3) all Done. Per §5 / §7 candidate ordering, the next M2 checkpoint is one of: **minimal rule/scorer/solver integration**, **local run artifact basics**, or **snapshot-extraction Apps Script implementation** (the last pinned to late-M2 per `docs/decision_log.md` D-0036). Sequencing among the three will be picked at the next checkpoint scoping; until then no checkpoint is active.

## 9. Task list for the current checkpoint

No active checkpoint. M2 C3 (`Parser/normalizer implementation closure`) closed on 2026-04-27 (see §11 sign-off note). The next candidate checkpoints per §7 / §5 ordering are minimal rule/scorer/solver integration, local run artifact basics, and snapshot-extraction Apps Script implementation (the last pinned to late-M2 per `docs/decision_log.md` D-0036). Sequencing and the next checkpoint's task list will be scoped when activation lands.
...
This version reflects the 2026-04-27 closure of M2 C3 (`Parser/normalizer implementation closure`) on top of the 2026-04-26 closure of M2 C2 (`Selector contract closure`), the 2026-04-25 closure of M2 C1 (`Rule engine + scorer + solver contract closure`), and the 2026-04-22 closure of M1.1 (`Operator-facing launcher`), and is currently in the following state:
- active milestone `Minimal local compute pipeline` (M2)
- no active checkpoint; next M2 checkpoint to be scoped separately per §8 (candidates per §5 / §7 ordering: minimal rule/scorer/solver integration, local run artifact basics, snapshot-extraction Apps Script implementation per `docs/decision_log.md` D-0036)
