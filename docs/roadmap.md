...
M6 is provisionally framed around solver-side score-aware search scoped **LAHC-only** per the M5 closure thread (D-0065). Cloud Deep Solve + email-notification architecture + cloud-side FULL retention promotion of FW-0030 are explicitly carved off to FW or future milestones — they are not in M6 scope. Scoring-formulation rework is parked as FW-0033, also explicitly NOT in M6 scope. M6 framing decision (provisional D-0066) lands as its own direction-setting docs-only PR mirroring the D-0049 / D-0055 pattern; the activation PR pins M6 + M6 C1 with the standard Task 1/2/3 cadence per D-0064.
