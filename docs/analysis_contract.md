...
- **Downstream upload portal** (M5 C3 — likely a route on the existing launcher Web App per `docs/decision_log.md` D-0063, mirroring D-0046's writeback upload pattern): consumes a single `AnalyzerOutput` JSON file per §10, deserializes it, hands the `AnalyzerOutput` to `RMLib.renderAnalysis(...)` per `docs/analysis_renderer_contract.md` §11.3. The portal's form shape and operator UX are NOT governed by this contract.
