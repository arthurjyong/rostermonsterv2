...
### 2.1 Version history
- **v1 (2026-05-04):** initial analysis contract closure per `docs/decision_log.md` D-0056..D-0058. Input shape, `AnalyzerOutput` shape, pure top-K selection with selector-cascade tiebreak, Tiers 1–5 emission scope (Tier 6 deferred to FW-0032; Tier 7 renderer-derived).
- **v2 (2026-05-13, this PR — per `docs/decision_log.md` D-0073):** §10.6 `PerDoctorAggregates` redesigned per operator-feedback on the M7 dry-run cycle. Dropped `weekendCallCount` (Saturday/Sunday only with no PH support was operator-misleading) and `maxConsecutiveDaysOff` (counted any non-call day as "off" — but doctors still round on non-call days, so operator-misleading). Renamed `cumulativeCallPoints` → `totalCallPoints`. Added 7 new fields: `group` (parser-canonical group classification), `averageCallPointsPerCall` (null if callCount=0), `shortestCallGap` / `secondShortestCallGap` / `longestCallGap` (scorer-stride gap definition: Mon→Wed = 2 aligned with `spacingPenalty`; null when undefined), `callRequestsFulfilled` / `callRequestsUnfulfilled` (per-doctor CR honor counts matching the scorer's `cr_reward` semantics). §10.8 `EquityScalars` updated in lockstep: dropped `weekendCallCount` block, renamed `cumulativeCallPoints` → `totalCallPoints`. Field removals are NOT tolerance-additive per §14, hence the version bump.
...
- Downstream: the Apps Script analyzer renderer (M5 C2) consumes `AnalyzerOutput` and writes K roster tabs + 1 comparison tab. The upload portal landed as a route on the launcher Web App per `docs/decision_log.md` D-0063 — accepts a single `AnalyzerOutput` JSON file from the operator and hands it to the renderer. (Originally anticipated as a separate M5 C3 sub-PR; bundled into M5 C2 + dropped from the plan at C2 closure when the route covered the portal scope.)
...
- the upload portal's form shape, file-size caps, or operator-facing UX (the portal landed as a launcher Web App route per `docs/decision_log.md` D-0063; M5 C3 was originally framed for this and dropped at M5 C2 closure when the route covered the portal scope),
...
```
PerDoctorAggregates {
  group: string                                  // doctor's parser-canonical group classification — typically one of "ICU_ONLY" | "ICU_HD" | "HD_ONLY" for the cgh_icu_hd template (resolved via Doctor.groupId per `docs/domain_model.md` §7.3); operator-facing column to sort/filter the per-doctor table by cohort
  callCount: int                                 // count of CALL-slot assignments
  standbyCount: int                              // count of STANDBY-slot assignments
  totalCallPoints: number                        // sum over the doctor's CALL assignments of the per-day call-point weight at (slotType, dayIndex), where the weight is derived from snapshot.scoringConfigRecords.callPointRecords after the parser overlay (D-0037)
  averageCallPointsPerCall: number | null        // totalCallPoints / callCount when callCount > 0; null when callCount == 0 (undefined-not-zero per §10.6 null discipline below)
...
**Field-removal version bump (v1 → v2 per D-0073).** v1 emitted `weekendCallCount` and `maxConsecutiveDaysOff`; both were dropped in v2 because operator feedback during the M7 dry-run cycle established they were misleading. `weekendCallCount` was Saturday + Sunday only with no PH support — the scorer's `pointBalance*` components weight weekend AND PH at the same higher rate (via `WeekendOrPublicHoliday` rule pairs in icu_hd's template `defaultRule`), so a "weekend" metric that omits PH silently disagrees with the scorer's notion of high-weight days. `maxConsecutiveDaysOff` counted any day without CALL or STANDBY as "off", but in hospital practice doctors still round on non-call days — the metric was a stretch-of-no-call indicator misnamed as "days off". Net effect: an analyzer reader using v1 fields would silently corrupt operator-facing equity comparisons. Future PH support is parked as FW-0031; when it lands, an additive v3 bump may add `weekendOrPhCallCount` (the operator-meaningful framing) per §14's tolerance rule.

**Null-not-zero discipline.** `averageCallPointsPerCall` and the three gap fields use `null` (Python `None`, JSON `null`) when the underlying input is too sparse for the metric to be defined — never `0`. A doctor with 0 calls has *undefined* average call points (not "zero on average"); a doctor with 1 call has *undefined* shortest gap (only one date — no pair to form a gap); a doctor with 2 calls has a defined shortest gap = longest gap but *undefined* second-shortest gap (only one gap exists). The renderer surfaces null as `—` (em-dash) so operators visually distinguish "unavailable" from "actually zero". Mirrors the v1 PH-deferral discipline below.

**Gap semantics — scorer-stride convention.** `shortestCallGap` / `secondShortestCallGap` / `longestCallGap` use the same gap definition as `scorer/components.py:189` `spacingPenalty`: `gap = (date_next - date_prev).days` between consecutive CALL dates (sorted ascending). So Mon→Wed yields **gap 2**, Mon→Thu yields gap 3, Mon→Sun yields gap 6. This alignment matters because the scorer's penalty curve is `weight / 2^(gap-2)` for gap ∈ {2..6} — an operator reading "shortest gap = 2" in the per-doctor table can directly cross-reference the scorer's `spacingPenalty` breakdown and know that's the worst-case full-weight pair. Reporting filler-days (Mon→Wed = 1) would create a one-off mismatch with every spacingPenalty number the operator looks at.

**Call-request fulfilment semantics.** A CR is "fulfilled" when the candidate's assignment places the requesting doctor on ANY call-kind slot on the request's `dateKey`. Matches `scorer/components.py:240..261` `cr_reward` definition exactly — analyzer Tier 2 counts and the scorer's `crReward` contribution are bit-aligned by construction. Standby assignments on the CR date do NOT count as fulfilment (the operator-meaningful CR is "you're on the call slot you asked for", not "you're on the schedule that day"). Both fulfilled and unfulfilled counts are emitted so the operator sees the doctor's total CR ask AND the honor rate without recomputing.
...
EquityScalars {
  ...
  totalCallPoints: { stdev: number, minMaxGap: number, gini: number }
}
```

**v1 → v2 changes per D-0073** (in lockstep with §10.6 `PerDoctorAggregates` v2): dropped the `weekendCallCount` equity block (corresponding `PerDoctorAggregates` field dropped); renamed the `cumulativeCallPoints` block to `totalCallPoints` to match the §10.6 rename.

`publicHolidayCallCount` equity scalars are STILL not emitted in v2 (paired with the §10.6 `PerDoctorAggregates` PH-deferral note). When a future snapshot extension surfaces real PH metadata, an additive v3 bump may add a `weekendOrPhCallCount: { stdev, minMaxGap, gini }` block here in lockstep with the §10.6 PH-aware field.
...
- **Tier 2 — per-doctor equity** (`PerDoctorAggregates`, **v2 per D-0073**): `group` (parser-canonical classification); CALL / STANDBY counts; `totalCallPoints` (within-cycle load per doctor under operator-overlaid call-point weights); `averageCallPointsPerCall` (null when callCount=0); `shortestCallGap` / `secondShortestCallGap` / `longestCallGap` (scorer-stride day gaps between consecutive CALL dates; null when undefined); `callRequestsFulfilled` / `callRequestsUnfulfilled`. (No per-doctor opening-balance / end-of-cycle / delta breakdown in v2; snapshot has no per-doctor opening call-point balance — see §10.6 call-point source note. **No public-holiday metrics in v2** — §10.6 PH-deferral note; PH support parked as `docs/future_work.md` FW-0031.)
- **Tier 3 — equity scalars** (`EquityScalars`, **v2 per D-0073**): per-candidate stdev / min-max gap / Gini for `callCount` and `totalCallPoints`. (Weekend/PH equity scalars deferred in lockstep with the Tier 2 PH-field deferral.)
...
- The analyzer-engine implementation (`python/rostermonster/analysis/`) landed under M5 C1 (PR #111); the Apps Script analyzer renderer + upload portal landed under M5 C2 (PRs #114, #115, #116) — the originally-framed M5 C3 boundary was dropped at C2 closure (the portal scope was wholly covered by the launcher Web App route per D-0063); live operator validation is M5 C4.
