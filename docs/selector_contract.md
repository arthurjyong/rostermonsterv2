...
- **`solverStrategyConfig`** *(tagged union, optional for backward compatibility)*: the strategy-specific parameters actually used at run time (NOT a declaration of defaults — the resolved values that shaped the search, after CLI flag overrides per `docs/solver_contract.md` §12A.7 or default fallback per §12A.5). **Producer obligation (post-M6 C3 Task 1)**: any execution-layer code that populates `solverStrategy` MUST also populate `solverStrategyConfig` with the matching tagged-union variant — knowing "LAHC ran" without knowing "with which `historyListLength` / `idleThreshold` / `maxIters`" defeats the provenance purpose (default-vs-tuned runs become indistinguishable, breaking the M5 analyzer's calibration role per D-0066 sub-decision 7). The two fields MUST be populated together or omitted together; populating only one is a contract-breaking caller defect. **Consumer obligation**: readers MUST tolerate absence of either field (e.g., archival envelopes) by treating params as unknown — readers MUST NOT infer params from the strategy enumerant alone (in particular, MUST NOT default to §12A.5 LAHC defaults when params are absent — that would silently mask CLI-overridden runs as default runs). The shape is a tagged union discriminated by an embedded `strategy` field:
  ...
  - `{ "strategy": "LAHC", "lahcParams": { "historyListLength": int, "idleThreshold": int, "maxIters": int } }` — the resolved LAHC parameters used for this run. All three fields are required when this variant is present; field meanings per `docs/solver_contract.md` §12A.5.
