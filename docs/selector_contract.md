...
**Cross-field invariant**: when both `solverStrategy` and `solverStrategyConfig` are present on the same `runEnvelope`, `solverStrategyConfig.strategy` MUST equal `solverStrategy`. Mismatch is a contract-breaking caller defect (the execution layer that built the envelope is expected to populate both fields from the same resolved-strategy value).
