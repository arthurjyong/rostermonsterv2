...
- **`solverStrategy`** *(string enumerant, optional)*: identifies the strategy that produced the candidate set. Allowed values are exactly the registered-strategy ids in `docs/solver_contract.md` §11.1 — currently `"SEEDED_RANDOM_BLIND"` or `"LAHC"`. Absent on envelopes from runs that predate this field's introduction (e.g., archival v2-era envelopes from before M6 C3 Task 1 landed); v2-compliant readers MUST tolerate the missing case (treat strategy as unknown/legacy rather than rejecting the envelope).
...
Per §16.3, both fields are optional additions and do NOT require a `contractVersion` bump. Prior-version-targeted readers (v1-targeted or v2-targeted) MAY ignore them and still process the envelope correctly — strategy was implicit single-strategy in v1 and pre-M6 v2 era runs (only `SEEDED_RANDOM_BLIND` existed), so absence is unambiguous: a missing `solverStrategy` on a pre-M6 envelope means `"SEEDED_RANDOM_BLIND"` ran.
