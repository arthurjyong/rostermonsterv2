...
The selector MUST NOT generate `runId`, MUST NOT remap `candidateId` values from the solver's emission order, and MUST NOT alter `snapshotRef` / `configRef` / `seed` / `fillOrderPolicy` / `crFloorMode` / `crFloorComputed` / `generationTimestamp` / `sourceSpreadsheetId` / `sourceTabName` / `solverStrategy` / `solverStrategyConfig` values received on the run envelope. The selector is a propagator of identity, not an originator.

### 16.5 Run-level solver-strategy metadata (optional, additive per §16.3)
*Added in M6 C3 Task 1 per `docs/decision_log.md` D-0067 sub-decision 8 (envelope-shape additive bump location).*

Two optional fields on `runEnvelope` record which solver strategy ran and its actual run-time parameters, so downstream consumers (the M5 analyzer per `docs/analysis_contract.md`, future ops-trail readers, reproducibility tooling) can identify the producer without parsing per-batch `SearchDiagnostics`:

- **`solverStrategy`** *(string enumerant, optional for backward compatibility)*: identifies the strategy that produced the candidate set. Allowed values are exactly the registered-strategy ids in `docs/solver_contract.md` §11.1 — currently `"SEEDED_RANDOM_BLIND"` or `"LAHC"`. **Producer obligation (post-M6 C3 Task 1)**: any execution-layer code that constructs a `runEnvelope` after this field landed MUST populate it with the actual strategy that ran — there is no ambiguity at the producer side under solver_contract v2 because `strategyId` is already a required input to `solve(...)` per `docs/solver_contract.md` §9. **Consumer obligation**: readers MUST tolerate absence (e.g., archival envelopes from runs that predate this field's introduction, or non-conforming third-party producers) by treating the strategy as **unknown/legacy** — readers MUST NOT infer a specific strategy from absence (in particular, MUST NOT default to `"SEEDED_RANDOM_BLIND"`). The optional-not-required framing exists to keep the change additive per §16.3 and to retain back-compatibility with archival envelopes; it is NOT a license for new producers to omit the field.

- **`solverStrategyConfig`** *(tagged union, optional for backward compatibility)*: the strategy-specific parameters actually used at run time (NOT a declaration of defaults — the resolved values that shaped the search, after CLI flag overrides per `docs/solver_contract.md` §12A.7 or default fallback per §12A.5). **Producer obligation (post-M6 C3 Task 1)**: any execution-layer code that populates `solverStrategy` MUST also populate `solverStrategyConfig` with the matching tagged-union variant — knowing "LAHC ran" without knowing "with which `historyListLength` / `idleThreshold` / `maxIters`" defeats the provenance purpose (default-vs-tuned runs become indistinguishable, breaking the M5 analyzer's calibration role per D-0066 sub-decision 7). The two fields MUST be populated together or omitted together; populating only one is a contract-breaking caller defect. **Consumer obligation**: readers MUST tolerate absence of either field (e.g., archival envelopes) by treating params as unknown — readers MUST NOT infer params from the strategy enumerant alone (in particular, MUST NOT default to §12A.5 LAHC defaults when params are absent — that would silently mask CLI-overridden runs as default runs). The shape is a tagged union discriminated by an embedded `strategy` field:
  - `{ "strategy": "SEEDED_RANDOM_BLIND" }` — no payload. `SEEDED_RANDOM_BLIND` has no strategy-specific knobs at the selector boundary; its inputs `fillOrderPolicy`, `crFloorMode`, `crFloorComputed` already ride through on `runEnvelope` per §16.2. The variant exists for symmetry so consumers can pattern-match on `strategy` without a special "no config" case.
  - `{ "strategy": "LAHC", "lahcParams": { "historyListLength": int, "idleThreshold": int, "maxIters": int, "swapProbability": float } }` — the resolved LAHC parameters used for this run. The first three fields are required when this variant is present; `swapProbability` was added during M6 C4 alongside the `--lahc-swap-probability` CLI knob and is OPTIONAL on the wire for backward compatibility with M6-C3-era envelopes — readers encountering an `lahcParams` payload that lacks `swapProbability` SHOULD interpret it as `0.5` (the only value pre-M6-C4 runs could have used, since the swap-vs-reassign coin flip was hardcoded `rng.random() < 0.5`). Post-M6-C4 producers MUST populate `swapProbability` for the same reason the other three fields are required: knowing "LAHC ran" without knowing the resolved move-mix bias would silently mask CLI-overridden runs as default runs. Field meanings per `docs/solver_contract.md` §12A.5 / §12A.7.

**Cross-field invariants**: (i) when both `solverStrategy` and `solverStrategyConfig` are present on the same `runEnvelope`, `solverStrategyConfig.strategy` MUST equal `solverStrategy` — mismatch is a contract-breaking caller defect (the execution layer is expected to populate both fields from the same resolved-strategy value); (ii) per the producer obligations above, post-M6 C3 Task 1 producers MUST populate the two fields **together** or **omit them together** — emitting only one (e.g., `solverStrategy: "LAHC"` without `solverStrategyConfig`) is a contract-breaking caller defect for the same provenance reason that motivates the joint MUST.

Per §16.3, both fields are optional additions and do NOT require a `contractVersion` bump. Prior-version-targeted readers (v1-targeted or v2-targeted) MAY ignore them and still process the envelope correctly. **Note on inference from absence**: it is tempting to read "absent on pre-M6 envelopes" as "absent ⇒ `SEEDED_RANDOM_BLIND` ran" because pre-M6 was a single-strategy era. This contract explicitly **forbids** that inference. The optional-additive framing means a post-M6 envelope MAY also lack the field (e.g., a non-conforming producer that ignores the producer obligation above, or an envelope from a tooling pipeline that wasn't updated). Conflating "this is a pre-M6 envelope" with "this lacks the field" silently misclassifies post-M6 LAHC runs as the baseline strategy — the exact provenance failure this field exists to prevent. Readers MUST treat absence as unknown/legacy and surface that to the operator (e.g., "strategy not recorded — re-run to populate") rather than guessing.

**Why a tagged union rather than two flat fields like `{ solverStrategy, lahcParams }`**: the tagged form scales cleanly when M6's successor strategies register under §11.2's extension clause. A hypothetical future `TABU_SEARCH` strategy adds `{ "strategy": "TABU_SEARCH", "tabuParams": {...} }` per its own strategy descriptor without touching this contract — the envelope shape stays a single tagged union and the discriminator inside the value is what makes the extension cleanly additive. Flat fields would require the envelope to grow a new top-level field (`tabuParams`) per strategy, multiplying surface area and forcing every reader to learn every strategy's parameter shape rather than dispatching on the tag.

**Why `runEnvelope` rather than `searchDiagnostics`**: `searchDiagnostics` per `docs/solver_contract.md` §18.1 is per-batch solver-stage transparency that already records `strategyId`. But §16.2 establishes that **run-level metadata flows once** on the `runEnvelope` at the `FinalResultEnvelope` level, not per-batch. Consumers needing run-level identity (which strategy ran for THIS run, for filtering / display / archival) read `runEnvelope`, not per-batch diagnostics. The two surfaces are not redundant: `searchDiagnostics.strategyId` describes what the solver consumed; `runEnvelope.solverStrategy` describes what the run is identified as. Within a contract-compliant run they will agree, but they live at different scopes.
...
### M6 C3 Task 1 follow-on (2026-05-08)
- `runEnvelope` gains two **optional** fields per new §16.5: `solverStrategy` (strategy enumerant per `docs/solver_contract.md` §11.1) and `solverStrategyConfig` (tagged union of strategy-specific actual params used at run time). Driven by `docs/decision_log.md` D-0067 sub-decision 8 (envelope-shape additive bump location settled at M6 C3) so downstream consumers can identify which solver strategy ran for run-level filtering / display / archival without parsing per-batch `searchDiagnostics`.
- `contractVersion` is **NOT** bumped — both fields are optional additive per §16.3, prior-version-targeted readers can ignore them and still process the envelope correctly.
- §9 item 3 is updated with a forward-pointer to §16.5; §16.2 metadata-flows-once enumeration is updated; §16.4 selector-doesn't-mutate enumeration is updated.
- Implementation lands in M6 C3 Task 2A (CLI flag plumbing) + 2B (integration tests). Cloud-mode LAHC integration carved off to FW-0035 per D-0068; cloud HTTP wrapper continues to ship `SEEDED_RANDOM_BLIND`-only in v1 — when the cloud wrapper later adopts LAHC per FW-0035's revisit triggers, this field's shape is already pinned and no further selector-contract change is required.
