...
- **Per-task retry policy:** **NO retry** (`taskGroups[0].taskSpec.maxRetryCount: 0`). Codex P2 round 8 amendment — pre-amendment setting was `maxRetryCount: 1`, but with `maxRunDuration: 540s` per attempt × 2 attempts = 1080s ≈ 18 min total wall, which blows the 10-min hard cap per D-0071 sub-decision 7. Single-attempt semantics keep the actual ceiling at 540s + small Cloud Batch scheduling slop ≤ 600s. The original "transient VM stalls" rationale for retry=1 was weighted against pilot scope (≤ a few runs/day; transient stalls rare on `c3-highcpu-88` on-demand allocation); accepting "operator manually re-clicks Solve Roster if the task fails" is the v1 fallback. If retry becomes load-bearing at scale, the finalize step would need to persist intermediate state to GCS as a checkpoint surface so retry can resume cheaply (future-work) AND the wall-budget framing would need to switch to "elapsed-since-RM_SUBMIT_TIMESTAMP_MS hard-stop in the finalize step" rather than "first-attempt-fits-540s".
...
- **Downstream** (callee): launcher Web App in `apps_script/launcher/src/` adds a new `async-render-callback` action handler that parses the POST body (JSON), then dispatches per `state`:
  - **`state = "OK"`**: invoke `RMLib.applyWriteback(JSON.stringify(callbackBody.writebackEnvelope))` (writes the writeback tab) + `RMLib.renderAnalysis(callbackBody.analyzerOutput)` (writes K roster tabs + 1 comparison tab) + `MailApp.sendEmail(operatorEmail, ...)` success notification.
  - **`state = "UNSATISFIED"`**: invoke `RMLib.applyWriteback(JSON.stringify(callbackBody.writebackEnvelope))` only (writes the failure-branch tab per `docs/writeback_contract.md` §13); SKIP `RMLib.renderAnalysis` because `analyzerOutput` is null on this path (Codex P2 round 8 finding 17 fix); send `MailApp.sendEmail` unsatisfied notification.
