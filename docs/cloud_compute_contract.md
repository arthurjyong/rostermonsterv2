...
- **Downstream** (callee): launcher Web App in `apps_script/launcher/src/` adds a new `async-render-callback` action handler that invokes `RMLib.applyWriteback(envelope) + RMLib.renderAnalysis(envelope, analyzerOutput, sourceSpreadsheetId)` from the central library against the callback's POSTed envelope, then calls `MailApp.sendEmail(operatorEmail, ...)` to notify the operator before returning 2xx to the caller.
