...
- **Total wall budget (D-0071 sub-decision 7 + Codex P1 round 10 + P2 round 12 + P2 round 15 + P2 round 3-on-PR-#171 amendments):** the **finalize-step self-check at 510s elapsed since `RM_SUBMIT_TIMESTAMP_MS`** is the hard mechanism that bounds the TASK — if the Pool finishes after it, the finalize step skips aggregation and POSTs a timeout-failure callback. The task's `maxRunDuration: 660s` is INTENTIONALLY above the 510s self-check + finalize budget so the self-check (NOT Batch) determines when the operator gets the timeout email; the 660s Batch limit only fires in the degenerate case where the Pool hangs indefinitely without raising (and that case falls into FW-0039). Cloud Batch's `Job` schema has no job-level `maxRunDuration` field; the task-level 660s is the Batch-level upper bound. **Important nuance (Codex P2 round 3 on PR #171):** the 510s self-check bounds the TASK, NOT the success-path operator email. On the SUCCESS path the operator email lands at `Pool-finish + finalize-work`, where finalize-work ≈ ~50s aggregate/score/select/analyzer + ~56s launcher-side `applyWriteback` + `renderAnalysis` + `sendEmail` ≈ 106s — so a Pool finishing near the 510s threshold yields an operator email ~616s after submit, modestly past the 600s "10-min" target. That ~15-20s success-path overrun is a **pre-existing property of where the threshold sits relative to the true finalize cost** (re-tuning it is a focused follow-up, not in scope for the operator-UX work that surfaced it). It is **independent of the callback-POST per-attempt timeout** (`_CALLBACK_POST_TIMEOUT_SECONDS`), which bounds the WORKER's patience, NOT the launcher's work — the launcher completes `applyWriteback` + `renderAnalysis` and emails the operator on its own schedule mid-POST regardless of when (or whether) the worker stops waiting. Operator-facing 10-min figure is a **target**, not a hard ceiling; the 510s self-check is the hard mechanism (it governs the TIMEOUT-failure path).
...
- `<launcher-callback-web-app-url>` is a **SECOND launcher Web App deployment** distinct from the existing operator-facing deployment per D-0071 sub-decision 4 — the new deployment is configured as `executeAs: "USER_DEPLOYING"` so the callback handler runs as the script owner (who has edit access to bound spreadsheets at pilot scope). The existing operator-facing launcher deployment (URL today: see project memory) stays as `executeAs: "USER_ACCESSING"` for the `/exec`, `/exec?action=writeback`, and `/exec?action=analysis-render` routes — operator browser sessions continue to carry their Drive permissions there. The Cloud Run Service propagates the callback deployment's URL into the Cloud Batch job spec as the env var `RM_LAUNCHER_CALLBACK_URL` on the finalizer task; the finalizer reads it at runtime. **Required code-side gate in `Launcher.gs` `doGet(e)` + `doPost(e)` (Codex P1 round 5 finding 9 amendment, security-critical):** because `executeAs: USER_DEPLOYING` + `Access: ANYONE` on the new deployment grants script-owner privileges to anyone with the URL, the `doGet`/`doPost` dispatchers MUST gate by deployment URL: read `ScriptApp.getService().getUrl()` and compare against `PropertiesService.getScriptProperties().getProperty('CALLBACK_DEPLOYMENT_URL')`. When the two match (running under the callback deployment), `doGet(e)` MUST reject ALL requests with `{state: "INVALID_DEPLOYMENT", code: "GET_NOT_ALLOWED_ON_CALLBACK_DEPLOYMENT"}` (callback is POST-only; allowing GET would fall through to the default launcher form whose `google.script.run` handlers would execute under the script owner's privileges) and `doPost(e)` MUST reject any request whose `e.parameter.action !== 'async-render-callback'` with `{state: "INVALID_DEPLOYMENT", code: "NON_CALLBACK_ROUTE_REJECTED"}`. The existing operator-facing routes (default sheet-gen, writeback form, analysis-render form) continue to work on the USER_ACCESSING deployment URL unchanged. **Pre-amendment language ("no code-side branching needed") was wrong** — code-side branching IS needed to isolate the callback route's elevated privileges to its single intended use case (POST from the Cloud Batch finalizer); the absence of branching would expose every operator-facing route under script-owner privileges via the callback deployment URL.
...
- Same `doPost(e)` dispatcher serves both. The deployment URL determines runtime identity via Apps Script's `executeAs` config + a required code-side gate at the top of `doGet(e)` / `doPost(e)` per the §10A.3 description — the gate isolates the callback route's elevated USER_DEPLOYING privileges so the `Access: ANYONE` setting on the new deployment can't be exploited via the operator-facing routes. **Pre-amendment language ("no code-side branching needed") was wrong** (Codex P1 round 5 finding 9 fix); code-side branching is security-critical.
...
```json
{
  ...
  "sourceSpreadsheetId": "<source Google Sheets spreadsheet ID>" | null
}
```
...
11. **`sourceSpreadsheetId`** *(optional, nullable string)* — the source Google Sheets spreadsheet ID, sourced directly from `snapshot.metadata.sourceSpreadsheetId` by the worker's finalize step. Carried on **every** callback state (`OK` / `UNSATISFIED` / `COMPUTE_ERROR`) — including the `COMPUTE_ERROR` path where `writebackEnvelope` is null — so the launcher's callback handler can build a direct spreadsheet link in the operator email without digging through the nested `writebackEnvelope`. The launcher constructs `https://docs.google.com/spreadsheets/d/<id>/edit` from this field for the email body's link line. **Additive-optional per §10A.8: does NOT bump `schemaVersion`.** A launcher running against an older worker build that doesn't send the field MUST gracefully omit the email's link line (treat absent / null / non-string as "no link"), NOT reject the callback — this keeps rolling deploys safe in both directions.
...
**Finalizer-side handling of callback POST failures:**
...
- On 5xx response: **no retry — terminal on the first attempt.** (Retry count went 3 → 1 → 0 across the post-M7 callback-budget rebalance. The original 3-retry value — D-0071 sub-decision 10 — was sized for the old 30s per-attempt timeout, where attempt 1 routinely timed out against a healthy-but-slow launcher (`renderAnalysis` runs ~37-42s synchronously inside the callback handler) so retries were load-bearing just to get a callback through. Two reasons 0 is now correct: (1) the retry is effectively vestigial — the launcher completes its work and emails the operator synchronously during the first POST attempt, so a retry only re-POSTs into the launcher's idempotency guard and gets `DUPLICATE_IGNORED` without changing the operator outcome; (2) the worst-case callback chain has to fit the ~150s finalize window between the 510s self-check threshold and the 660s `maxRunDuration`, and even one retry at the 60s timeout is a ~122s chain that overruns once ~50s of aggregate/score/select/analyzer is also counted. 0 retries caps the callback at a single 60s attempt. See the §12 change log.) Failure on the single attempt is treated as terminal "launcher unreachable" — finalizer logs the failure to Cloud Logging with the full envelope (runId, attemptId, state, error, diagnostics) + exits cleanly. **The operator is NOT emailed in this terminal-unreachable path** because the finalizer has no mail surface (see "Operator-email gap on callback-unreachable" below); the gap is accepted-for-v1 per `docs/future_work.md` FW-0039 + D-0071 sub-decision 17.
- On 401: terminal, no retry (token issue indicates config drift). Finalizer logs to Cloud Logging + exits cleanly. No operator email (same gap as the 5xx case).
...
All three email bodies include a **direct link to the source spreadsheet** (`https://docs.google.com/spreadsheets/d/<id>/edit`) near the top, built from the §10A.6 `sourceSpreadsheetId` callback field. The link is most load-bearing on the failure path — on a `COMPUTE_ERROR` nothing changed in the operator's sheet, so they likely don't have it open, yet the email tells them to retry from there. When `sourceSpreadsheetId` is absent (older worker build mid-rolling-deploy), the launcher omits the link line gracefully.
- **Success path (`state = "OK"`)**: Subject `[RosterMonsterV2] Roster solve complete — runId <short-id>`. Body: short summary + spreadsheet link. **Attachment: full `AnalyzerOutput` JSON** per D-0071 sub-decision 5 (forensic recovery value if a rendered tab gets accidentally deleted; ~50-200 KB typical, well under Gmail's 25 MB limit; Codex P2 round 17 fix).
- **Unsatisfied path (`state = "UNSATISFIED"`)**: Subject `[RosterMonsterV2] Roster solve unsatisfied — runId <short-id>`. Body: explains "no valid roster found under the request constraints" + the unfilled-demand summary from the writebackEnvelope's UnsatisfiedResultEnvelope + spreadsheet link. No attachment (no AnalyzerOutput on this path; analyzer doesn't run when K'=0 per `docs/analysis_contract.md` §9.2).
- **Failure path (`state = "COMPUTE_ERROR"`)**: Subject `[RosterMonsterV2] Roster solve failed — <error.code>`. Body: short human-readable explanation + the `runId` + `attemptId` + `wallTimeSeconds` + spreadsheet link + a maintainer contact line. **Attachment**: the full `AnalyzerOutput` JSON when present (rare on COMPUTE_ERROR — analyzer typically doesn't run, but include if the defect happened post-analyzer e.g. callback POST issue). Max attachment size capped by Gmail's 25 MB limit; AnalyzerOutput is typically ~50-200 KB.
...
- Multi-operator scale where the launcher's `USER_DEPLOYING` callback deployment's script-owner identity (`maintainer@example.com` at pilot scope) does NOT have edit access to every operator's bound spreadsheet — accepted-for-v1 since pilot scope is single-operator = script-owner. Promotable as a future FW entry when concrete multi-operator demand surfaces.
