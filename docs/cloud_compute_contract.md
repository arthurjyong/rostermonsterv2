...
**Manifest + deploy-dialog mechanic (Codex P1.6 finding clarification):** the launcher's `apps_script/launcher/src/appsscript.json` manifest's `webapp.executeAs` stays as `USER_ACCESSING` — that value is the *default* for new deployments made via `clasp deploy` (which the existing operator-facing deployment uses for its pinned `@17` + future bumps). The new callback deployment is created **via the Apps Script editor's "Deploy → New deployment" dialog** with the `Execute as` dropdown manually overridden to `Me`. Apps Script pins each deployment's `executeAs` at deploy time independent of the manifest's value. Two deployments → two URLs → same script project, same `doPost(e)` code, different runtime identities determined by which URL the request hit. No `clasp` plugin / programmatic config needed for the override; it's a one-time editor UI action at M7 C4 deployment time.
