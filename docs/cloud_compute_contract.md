...
### 10.4 Determinism
The service is deterministic in the same sense the local CLI is per `docs/selector_contract.md` §18: same `(snapshot, optionalConfig)` input produces byte-identical `writebackEnvelope` across re-runs. The cloud service inherits this guarantee by sharing the compute core with the local CLI per D-0050; no separate determinism guarantee is offered.
...
Cloud-mode runs and local-mode runs at the same `(snapshot, optionalConfig)` produce byte-identical wrapper envelopes. This is guaranteed by D-0050's shared-compute-core architecture: both invocation paths reach the same Python compute code with the same inputs. Maintainer-side solver-strategy experiments performed via the local CLI are guaranteed reproducible in the cloud, and any cloud-mode defect is reproducible from the local CLI.
