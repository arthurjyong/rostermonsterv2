...
- **Total wall budget (D-0071 sub-decision 7 + Codex P1 round 10 amendment):** the **finalize-step self-check at 600s elapsed since `RM_SUBMIT_TIMESTAMP_MS`** is the load-bearing operator-facing ceiling (Codex P1 round 10 — the original framing made `maxRunDuration` the ceiling, but Batch would then kill the task before the finalize step could send a timeout email on slow Pool runs). The task's `maxRunDuration: 660s` is INTENTIONALLY above 600s as a safety net for the degenerate case where the Pool hangs without raising. Cloud Batch's `Job` schema has no job-level `maxRunDuration` field; the task-level 660s is the Batch-level upper bound, and the 600s finalize self-check is the operator-facing ceiling.
