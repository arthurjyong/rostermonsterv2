...
- **Per-task timeout:** **540s** (`taskGroups[0].taskSpec.maxRunDuration: "540s"`) per **D-0071 sub-decision 7** (M7 C3 amendment). Pre-D-0071 setting was 180s under the sync framing; D-0071's async pivot unblocks this to 540s (9 min). Covers VM provisioning ~30-60s + Pool(88) LAHC trajectories ~360-450s (real production roster at the FW-0037 elbow tuple `L=50`/`idleThreshold=3500`) + inline finalize step ~30-60s (aggregate + score + select + analyzer + callback POST + email) + buffer. The 540s setting fits inside the 10-min total wall budget enforced via the finalizer-step self-check (see "Finalizer-inline pattern" below).
- **Per-task retry policy:** **1 retry on failure, fail-fast on second** (`taskGroups[0].taskSpec.maxRetryCount: 1`). Default Cloud Batch retry is 0; a single retry shrugs off transient VM stalls without doubling worst-case wall time on every run. Note: with a single VM doing everything, a retry restarts BOTH the trajectories AND the finalize step from scratch (the previous attempt's in-memory state is gone). At pilot scope (≤ a few runs/day) this is acceptable; if retry cost becomes meaningful, the finalize step can persist intermediate state to GCS as a checkpoint surface (future-work).
...
- **Finalizer-inline pattern (D-0071 sub-decision 2 + 12, Codex P1.7 amendment):** the finalize step (aggregate + score + select + analyzer + callback POST) runs **inside the same Cloud Batch task as the LAHC trajectories**, in the same Python process, AFTER `multiprocessing.Pool.close() + .join()` returns. No second task group, no dependency mechanic, no cross-task GCS aggregation step. The task spec:
  ...
  1. **Total-wall self-check (D-0071 sub-decision 7):** first action of the finalize step is to compare current wall time vs `RM_SUBMIT_TIMESTAMP_MS`. If `(now_ms - RM_SUBMIT_TIMESTAMP_MS) > 600_000` (10 min), the worker SKIPS aggregation/scoring/render entirely and POSTs the timeout-failure callback to the launcher (which then emails operator with the cost-mention failure body per §10A.7 + D-0071 sub-decision 5). Defense-in-depth — normally won't fire since the task's own 540s `maxRunDuration` is below 600s — but covers the edge case where Cloud Batch's task scheduling overhead pushes wall past 600s.
...
**Partial-failure tolerance (amended at M7 C3 Codex P1.7 fix):** under the single-task pattern, there is no cross-task failure aggregation — only intra-task per-trajectory drop-and-continue inside the Pool per `docs/solver_contract.md` §12A.8. If the single Batch task itself fails after Cloud Batch's `maxRetryCount: 1` retry exhausts, the WHOLE run fails (no surviving partial results, the entire VM died). If individual trajectories within the Pool raise per-trajectory seed-construction failures, the Pool's `imap_unordered` continues and the finalize step aggregates only the successful trajectories' candidates. K' = `len(candidates returned by Pool.imap_unordered)`.
...
**Authentication flow:**
- Finalizer task generates a Google OIDC ID token via the metadata server's `instance/service-accounts/default/identity?audience=<launcher-callback-web-app-url>` endpoint (Cloud Batch VMs run as the default Compute Engine SA per §8.7; the metadata server is available on the VM by default). The token's `aud` claim is the callback deployment's Web App URL (NOT the operator-facing deployment's URL).
...
- The callback deployment can run in **HEAD mode** (no version pinning) since the maintainer is its only consumer. Operator-facing deployment stays pinned (`@17` + future bumps) for stability.
...
6. **`state`** *(required, enum string)* — `"OK"` / `"UNSATISFIED"` / `"COMPUTE_ERROR"`. Same semantics as §10.1's response states minus `SUBMITTED` (the front-door already returned that) and minus `INPUT_ERROR` (input validation runs synchronously at the front door; if it fails the finalizer never runs). `COMPUTE_ERROR` here covers post-submission failures: worker tasks failed entirely (K'=0), aggregation raised, analyzer raised, or the finalizer itself caught an unhandled exception.
7. **`writebackEnvelope`** *(nullable)* — present when `state ∈ {"OK", "UNSATISFIED"}`. Same shape as §10.1's `writebackEnvelope`; the launcher hands directly to `RMLib.applyWriteback(envelope)`.
8. **`analyzerOutput`** *(nullable)* — present when `state = "OK"` (per D-0071 sub-decision 12 + the §10.6 OK-only convention). Same shape as `docs/analysis_contract.md` §4 `AnalyzerOutput`; the launcher hands directly to `RMLib.renderAnalysis(envelope, analyzerOutput, sourceSpreadsheetId)`. Null when `state ∈ {"UNSATISFIED", "COMPUTE_ERROR"}` (analyzer doesn't run on failure paths).
9. **`error`** *(nullable)* — present when `state = "COMPUTE_ERROR"`. Same shape as §10.1's `error`. Carries machine-readable `code` + human-readable `message`. First-release codes: `WORKER_ALL_FAILED` (K'=0; all worker tasks failed entirely), `AGGREGATION_EXCEPTION`, `ANALYZER_EXCEPTION`, `FINALIZER_EXCEPTION`.
...
- **Success path (`state = "OK"`)**: Subject `[RosterMonsterV2] Roster solve complete — runId <short-id>`. Body: short summary + link to the spreadsheet (sourced from `snapshot.metadata.sourceSpreadsheetId`). No attachment.
- **Unsatisfied path (`state = "UNSATISFIED"`)**: Subject `[RosterMonsterV2] Roster solve unsatisfied — runId <short-id>`. Body: explains "no valid roster found under the request constraints" + the unfilled-demand summary from the writebackEnvelope's UnsatisfiedResultEnvelope. No attachment.
- **Failure path (`state = "COMPUTE_ERROR"`)**: Subject `[RosterMonsterV2] Roster solve failed — <error.code>`. Body: short human-readable explanation + the `runId` + `attemptId` + `wallTimeSeconds` + a maintainer contact line. **Attachment**: the full `AnalyzerOutput` JSON when present (rare on COMPUTE_ERROR — analyzer typically doesn't run). Max attachment size capped by Gmail's 25 MB limit; AnalyzerOutput is typically ~50-200 KB.
...
- §8.7 (worker+finalize task) — per-task `maxRunDuration` raised from `180s` → `540s` per D-0071 sub-decision 7 (covers Pool(88) trajectories + inline finalize step). **Total wall budget enforced via the 540s task `maxRunDuration` as the hard ceiling**; defense-in-depth finalize-step self-check at 600s elapsed since `RM_SUBMIT_TIMESTAMP_MS` (set by Cloud Run Service at `submitJob` time and passed via the task's env) covers the edge case where Cloud Batch scheduling overhead pushes total wall past 600s. Pre-Codex P1.7's separate finalizer task group with its own `maxRunDuration: 120s` + cross-group `SUCCEEDED_OR_FAILED` dependency is RETIRED — that mechanic isn't supported by Cloud Batch v1.
