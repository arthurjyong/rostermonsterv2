...
- **Downstream** (callee): launcher Web App in `apps_script/launcher/src/` adds a new `async-render-callback` action handler that parses the POST body (JSON), then invokes `RMLib.applyWriteback(JSON.stringify(callbackBody.writebackEnvelope))` + `RMLib.renderAnalysis(callbackBody.analyzerOutput)` from the central library. **Signature notes (Codex P1 round 7 fix):** `RMLib.applyWriteback` accepts a JSON STRING and parses internally (per `apps_script/central_library/src/Writeback.gs:23-26`; the launcher MUST re-stringify the envelope object after parsing the POST body); `RMLib.renderAnalysis` accepts a single `AnalyzerOutput` argument (object OR string; library auto-parses if string per `apps_script/central_library/src/AnalysisRenderer.gs:37-49`); target spreadsheet ID is derived from `analyzerOutput.source.sourceSpreadsheetId` internally — no separate spreadsheetId arg. Then calls `MailApp.sendEmail(operatorEmail, ...)` to notify the operator before returning 2xx to the caller.
