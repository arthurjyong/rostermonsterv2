...
- On 5xx response: finalizer retries **once** with a 2s backoff. (Reduced from 3 retries — the original D-0071 sub-decision 10 value — as a budget-fit follow-up when the per-attempt callback timeout was bumped 30s → 90s. The 3-retry budget was sized for the old 30s timeout, where attempt 1 routinely timed out against a healthy-but-slow launcher (`renderAnalysis` runs ~37-42s synchronously inside the callback handler). With a 90s timeout attempt 1 succeeds against any healthy launcher, so a retry now only covers a genuine transient 5xx blip; keeping 3 retries would have made the worst-case POST chain ~4 × 90s + 14s ≈ 374s — long enough to risk a Cloud Batch task kill before the chain exhausts. 1 retry caps the worst case at ~2 × 90s + 2s ≈ 182s. See the §12 change log.) Failure after the retry is treated as terminal "launcher unreachable" — finalizer logs the failure to Cloud Logging with the full envelope (runId, attemptId, state, error, diagnostics) + exits cleanly. **The operator is NOT emailed in this terminal-unreachable path** because the finalizer has no mail surface (see "Operator-email gap on callback-unreachable" below); the gap is accepted-for-v1 per `docs/future_work.md` FW-0039 + D-0071 sub-decision 17.
