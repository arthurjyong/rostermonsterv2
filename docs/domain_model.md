...
- All `AssignmentUnit` entries that share the same `(dateKey, slotType)` pair (i.e., differ only by `unitIndex`) are **equivalent for doctor-admissibility rules** — the four doctor-admissibility hard invariants (`BASELINE_ELIGIBILITY_FAIL`, `SAME_DAY_HARD_BLOCK`, `SAME_DAY_ALREADY_HELD`, `BACK_TO_BACK_CALL` per `docs/rule_engine_contract.md` §11) evaluate identically regardless of which `unitIndex` the `proposedUnit` refers to — and are equivalent for baseline workload weight. The per-unit occupancy invariant `UNIT_ALREADY_FILLED` is explicitly NOT part of this equivalence: it is per-unit occupancy bookkeeping keyed on the full `(dateKey, slotType, unitIndex)` identity and correctly branches on `unitIndex` by construction (it is exactly how `requiredCount > 1` multiplicity is tracked). `unitIndex` is a stable operational identity that supports writeback labeling (for example, "MO1", "MO2", "MO3" mapping to `unitIndex = 0, 1, 2`); it MUST NOT carry implicit difficulty or workload differentiation. Departments that genuinely need semantically-differentiated roles MUST model them as distinct `SlotType` identities, not as distinct `unitIndex` values of the same `SlotType`. Per-position fairness across `unitIndex` values is a future additive scoring component (`docs/future_work.md` FW-0008), not a `unitIndex` weight; per-slot-type burden weighting is a future additive `SlotTypeDefinition` field (`docs/future_work.md` FW-0009). See `docs/decision_log.md` D-0029.
