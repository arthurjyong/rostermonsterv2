...
- **Outbound (writeback, M3 C1):** Python CLI emits a wrapper envelope JSON file (per `docs/decision_log.md` D-0045 — single file containing FinalResultEnvelope + snapshot subset + doctorIdMap; per D-0047 the CLI's standard output IS the writeback envelope). The operator uploads this file via the launcher Web App's writeback form (`<input type="file" accept=".json,application/json">`); the form posts the file content to `doPost(e)` which `JSON.parse`s and invokes the writeback library. Drag-and-drop, clipboard paste, and Drive Picker remain implementation-slice options for future polish (`docs/decision_log.md` D-0044 sub-decision 5).
