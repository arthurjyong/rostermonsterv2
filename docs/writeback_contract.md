...
1. **`finalResultEnvelope`** — a `FinalResultEnvelope` per `docs/selector_contract.md` §10. The selector's run envelope (`finalResultEnvelope.runEnvelope`) MUST carry the additive source-sheet identity fields specified in §18: `sourceSpreadsheetId` and `sourceTabName`. The `result` field is either an `AllocationResult` (success branch) or an `UnsatisfiedResultEnvelope` (failure branch); writeback handles both per §10, §11, and §13.
...
- The envelope MUST NOT carry `searchDiagnostics` content beyond the minimum failure-branch fields the writeback tab needs per §13 (`unfilledDemand`, `reasons`). Aggregated funnel counts, rejection histograms, and per-batch summaries are selector-stage retention surfaces, not writeback-input data.
- Writeback MUST NOT read any state outside the declared inputs and the live spreadsheet target it is asked to write to. No environment variables, no clocks (the run envelope's `generationTimestamp` is execution-layer-supplied per `docs/selector_contract.md` §16; writeback MUST NOT synthesize timestamps for traceability content), no filesystem reads beyond the JSON envelope handoff itself.
...
### 11.1 Tab name
The new tab MUST be named `<original-tab-name>_RM<YYMMDDHHMMSS>` where:
- `<original-tab-name>` is the value of `runEnvelope.sourceTabName`,
- `<YYMMDDHHMMSS>` is the second-resolution timestamp at writeback time, formatted in the operator's spreadsheet locale.

The second-resolution timestamp is sufficient to make tab-name collisions vanishingly rare in practice. Implementations MUST guarantee tab-name uniqueness within the spreadsheet by auto-suffixing `_2`, `_3`, … on the rare same-second collision. The contract requires uniqueness; the suffix scheme is the recommended mechanism but is not the only conformant option (any deterministic uniqueness mechanism that preserves the `_RM<YYMMDDHHMMSS>` prefix is acceptable).
...
Because the source tab is never mutated, the broader conflict-handling problem (operator edits between run-start and writeback) does not arise at the writeback boundary. Writeback always succeeds in producing a new tab; there is no detect-and-refuse, detect-and-warn, or blind-overwrite mode to surface. Operator-edited column-A names, request cells, call-point cells, or fixed-assignment prefills made between run-start and writeback do not propagate to the writeback tab — the writeback tab reflects run-start state per the snapshot bundle. This is an accepted consequence; both representations remain visible to the operator on different tabs.
...
| `contractVersion` | integer | `1` (constant from this contract's identity per §2) |
