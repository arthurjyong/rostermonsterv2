...
**Resolved transport (per `docs/decision_log.md` D-0040, settled in M2 C9; refined per D-0044, settled in M3 C1):** browser-download of a JSON file on the inbound side; **file upload via the launcher web form** on the outbound (writeback) side. Concretely:
- **Inbound (snapshot extraction, M2 C9):** Apps Script extractor emits a JSON blob that the operator saves locally; Python CLI ingests via `--snapshot path/to/file.json`.
- **Outbound (writeback, M3 C1):** Python CLI emits a wrapper envelope JSON file (per `docs/decision_log.md` D-0045 — single file containing FinalResultEnvelope + snapshot subset + doctorIdMap; per D-0047 the CLI's standard output IS the writeback envelope). The wrapper envelope is uploaded via the launcher Web App's writeback form (`<input type="file" accept=".json,application/json">`); the form's submit handler reads the file content client-side (browser `FileReader` API) and delivers the JSON string to a server-side handler — `google.script.run.<name>` for the embedded-HtmlService-form pattern OR `doPost(e)` for a true HTTP-POST form (mutually exclusive; the implementation picks one per `docs/decision_log.md` D-0044 sub-decision 3). Per **D-0074** the `Apply writeback` nav link was removed from the launcher UI — the `?action=writeback` route + `WritebackForm.html` are retained as a maintainer fallback (the route still resolves by direct URL), just no longer advertised, since cloud-mode applies writeback automatically. Drag-and-drop, clipboard paste, and Drive Picker remain implementation-slice options for future polish (`docs/decision_log.md` D-0044 sub-decision 5).
- UTF-8 JSON, pretty-printed (2-space indent), no envelope wrapping at the transport layer beyond the wrapper envelope shape declared in D-0045.

The §6.3 / §22 deferral language above is retained for forward-compat (drag-and-drop and Drive Picker remain implementation-slice options for future operator-UX polish), but the concrete first-release transport is now pinned on both directions; the joint-scoping commitment from D-0036 is fully resolved.

**M4 C1 cloud-mode invocation path (per `docs/decision_log.md` D-0049 / D-0050 / D-0052):** alongside the launcher Web App's file-upload route above (which is retained as the local-mode maintainer fallback — no longer nav-advertised per **D-0074**, but still resolvable by direct URL), the writeback library gains a second invocation path: direct in-Apps-Script invocation from the bound shim's "Solve Roster" menu handler. In that path, the bound shim assembles the wrapper envelope in-memory by orchestrating extract → cloud HTTP compute → in-memory writeback per `docs/cloud_compute_contract.md` (no file boundary mid-flow), and invokes `RMLib.applyWriteback(envelope)` directly. The writeback library itself is a pure adapter (per §6.2) and behaves identically in both invocation paths — same wrapper envelope shape per §9, same tab-creation semantics per §10–§11, same atomicity per §14, same diagnostic surface per §17. The two paths share the same library code (which moves into the central library `apps_script/central_library/` per D-0052 so both consumers can reach it). This addendum does NOT change the writeback contract's input shape or behavior — it only declares that an in-process invocation path exists alongside the file-upload-via-Web-App path. No `contractVersion` bump (additive — see §20.1's "do not bump for ... clarification that does not change behavior").
