...
## Current state
- **Live end-to-end in two modes.** Operator-facing one-click cloud (`Roster Monster → Solve Roster` menu in the bound-template spreadsheet → Cloud Run service `roster-monster-compute` → writeback into a new tab) and a maintainer-facing local CLI fallback (headless `clasp run extractSnapshotById` → `python -m rostermonster.run --snapshot file.json` → writeback envelope uploaded via the launcher's retained `?action=writeback` route). Per D-0074 the local-mode UI entry points were removed from the operator menus — the operator UI exposes only the one-click cloud path; the local-mode code + routes are retained as a maintainer fallback. Both modes share one Python compute core per D-0050 and produce byte-identical envelopes at the same input when seed is explicitly set per D-0053.
...
- **Active milestone: none** *(M7 closed 2026-05-13 per D-0072 — full async UX validated end-to-end on both real live ICU and HD operator cycles; the four happy-path operator-facing exit criteria from §7 C5 passed: bound shim's SUBMITTED toast → `[RosterMonsterV2] Success: <tab>` email with AnalyzerOutput JSON attachment → writeback + 5 roster tabs → **analyzer comparison tab** which closes the D-0069-deferred operator-cycle cross-reference pathway as a natural side-effect of M7 C4's inline-finalize-step analyzer plumbing; two stretch criteria — concurrent-rejection on double-click, parser-rejection still synchronous — have unit-test coverage from PR #157 but were not exhaustively live-validated)*. M7 closure trail: **D-0070 → D-0071 → D-0072**. M7 delivered async cloud LAHC via Cloud Batch single-VM dense pack (one `c3-highcpu-88` running `multiprocessing.Pool(88)` for K=88 LAHC trajectories + inline finalize step in `worker.py` after `Pool.close() + .join()`). Production operator flow: click `Roster Monster → Solve Roster` → toast appears → 3-10 min later receive email → open spreadsheet → see all expected tabs. FW-0037 elbow tuple locked at the M7 production config: `L=50` / `idleThreshold=3500` / `swapProbability=0.5`. K=88 reflects current C3_CPUS=108 quota; future quota bump unlocks K=176 (FW-0040 dial, stays parked). **FW-0027 (parallel operational search and orchestration), FW-0030 (cloud-side FULL retention support), FW-0035 (cloud-mode LAHC integration), and FW-0038 (parallel solver) all DELIVERED via M7 per D-0072**. FW-0039 (silent-operator-outcome watchdog) and FW-0040 (K=88 → K=176 single-VM dial) stay parked with their existing revisit triggers. **Active checkpoint: none** *(M7 C5 closed 2026-05-13 per D-0072; M7 milestone closed same day)*. Post-M7 deliberate study window mirrors the post-M4 (2026-05-01) / post-M6 (2026-05-09) cadence — no successor pre-decided. Candidate next-work threads: FW-0033 (scoring-weight elicitation), FW-0036/FW-0037 multi-fixture promotion (LAHC K-default + L-tuning protocol module-pinning waits for 2+ additional cycles), FW-0028 (observability hardening), FW-0031/FW-0032 (analyzer extensions), operational broadening beyond ICU/HD-first. End-to-end operator workflow live in cloud (LAHC at K=88 via `solverStrategy=LAHC` in the bound shim's POST body; legacy `SEEDED_RANDOM_BLIND` path stays sync for back-compat, hidden from operator menu per D-0071 sub-decision 13) and local (CLI with `--strategy LAHC` + analyzer + launcher upload form).
...
- `docs/decision_log.md` — Accepted directional decisions (D-0001..D-0074).
...
## Code layout
- `apps_script/launcher/` — Operator-facing launcher Web App (sheet generation + analysis-render route; the `?action=writeback` route is retained but no longer nav-linked per D-0074).
- `apps_script/bound_shim/` — Bound shim attached to the template spreadsheet (`Roster Monster → Solve Roster` menu; the `Extract Snapshot` item was removed per D-0074, handler retained).
