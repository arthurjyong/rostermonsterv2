...
- **All milestones M1 / M1.1 / M2 / M3 / M4 / M5 / M6 / M7 closed** (M7 closed 2026-05-13 per D-0072 — live async UX validated end-to-end on real ICU + HD operator cycles). Default `pytest` selects ~147 tests at M7 closure (50+ added across M7 C2's worker / orchestrator / batch job spec / audit modules plus 30+ in M7 C4 T2D's `test_service.py` thin-front-door coverage; slow tests opt-in via `pytest -m slow` for end-to-end audit at K=88 and full orchestrator integration). 17 Apps Script writeback unit tests.
