...
- **Active milestone:** **M7 — Parallel solver (Cloud Batch + intra-request K-fanout, async UX)** *(activated 2026-05-10 per D-0070; UX pivoted sync → async on 2026-05-11 per D-0071 driven by M7 C2 perftest evidence)*. M7 parallelizes K-trajectory LAHC via Cloud Batch + dense pack (`c3-highcpu-8` VMs × `multiprocessing.Pool(8)` × 1 LAHC trajectory per vCPU). Original M7 framing (D-0070) targeted sync UX inside a 250s Cloud Run timeout; M7 C2 perftest evidence (real production roster at K_approved=104) showed 5/13 tasks unfinished at the 240s deadline, returning partial K'=64. D-0071 pivots M7 C3 onward to async architecture: Cloud Run thin front door (validate + submit + return SUBMITTED ~3-5s) + Cloud Batch finalizer task group (aggregate + score + analyze + POST to launcher Web App callback + email operator) with 10-min hard cap. FW-0037 elbow tuple locked: `L=50` / `idleThreshold=3500` / `swapProbability=0.5`. K=2,500 target with smaller approvals acceptable since best-of-K plateau is K=10-20 per FW-0036. On-demand pricing chosen over Spot for predictable wall-time (cost is not a constraint at the doctor-time-comparison framing per the M7 design conversation). Cloud Run per-instance concurrency, threading, and Spot pricing explicitly excluded per D-0070 sub-decisions. FW-0035 (cloud-mode LAHC integration) + FW-0030 (cloud-side FULL retention support) co-promote with M7 closure (closure path now async per D-0071).
- **Active checkpoint:** **M7 C3 — Async architecture decision (D-0071)** *(activated 2026-05-11 on M7 C2 closure; scope pivoted from sync UX to async architecture decision on 2026-05-11 per the M7 C2 perftest evidence + D-0071)*. M7 C2 closed 2026-05-11 — full Cloud Batch parallel-LAHC chain delivered (worker + orchestrator + Batch job spec + GCS adapter + attempt-id race closure) with the §12A.4 byte-identity exit criterion proven (5/5 audit tests). M7 C2 perftest revealed sync-budget brittleness (5/13 tasks unfinished at 240s deadline on real production roster); D-0071 pivots M7 C3 to async architecture: Cloud Run thin front door (validate + submit + return SUBMITTED ~3-5s) + Cloud Batch finalizer task group (aggregate + score + analyze + POST to launcher Web App callback + email operator) with 10-min hard cap, GCP ID-token auth, B-prime callback delivery, always-email-on-failure, concurrent-rejection by spreadsheet ID. M7 C3 is docs-only — D-0071 + supporting sweeps. Subsequent M7 C4 (async UX implementation per D-0071) / M7 C5 (live operator validation + M7 closure) per `docs/delivery_plan.md` §7.
