...
- **Active checkpoint:** **M6 C3 — Envelope / CLI integration (local-first per D-0068).** Wraps LAHC into the local Python pipeline per D-0050's local-track. Task 1 envelope-shape additive bump (`solverStrategy` enumerant + actual params record); Task 2A CLI flags for strategy (`--strategy`, `--lahc-history-length`, `--lahc-iter-cap`, `--lahc-idle-threshold` per `docs/solver_contract.md` §12A.7); Task 2B integration tests against the local pipeline; Task 3 closure. M6 C1 closed 2026-05-07 per D-0067 (solver-strategy contract extension + LAHC algorithm spec; solver contract bumped `contractVersion: 1` → `2` for §9 input-shape change). M6 C2 closed 2026-05-08 (LAHC implementation in `python/rostermonster/solver/` per §12A spec — Task 2A strategy abstraction, Task 2B inner-loop, Task 2C K-trajectory + determinism + ICU/HD fixture integration tests; no contract changes). Cloud-mode LAHC integration parked as FW-0035 per D-0068. See `docs/delivery_plan.md` §7 / §9.
