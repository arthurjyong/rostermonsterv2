...
- **Active milestone:** **M6 — Solver-side score-aware search (LAHC)** *(activated 2026-05-07 same-day as M5 closure per D-0066)*. Scope is **LAHC-only** — Late Acceptance Hill Climbing as the first alternative solver search strategy alongside today's `SEEDED_RANDOM_BLIND`, addressing FW-0003's empirical score plateau. Strategy is internal to the solver boundary; two strategy-aware fields cross at v2 — `strategyId` as a required input per `docs/solver_contract.md` §9 (callers MUST pass it explicitly) and the wrapper envelope's `solverStrategy` enumerant as an output recording which strategy ran. Maintainer-only operator-tunable surface (Python module constants + CLI flag overrides; no operator-facing UI). Cloud Deep Solve + email-notification + cloud-side FULL retention promotion (FW-0030) + scoring-formulation rework (FW-0033) explicitly carved off to FW or future milestones.
- **Active checkpoint:** **M6 C4 — Live operator validation via M5 analyzer.** Operator runs both `SEEDED_RANDOM_BLIND` and `LAHC` on a real ICU/HD cycle, renders each `AnalyzerOutput` via the launcher's `?action=analysis-render` route, and manually cross-references the two resulting comparison tabs (per-component breakdowns + Hamming matrix + equity scalars). Verdict feeds: M6 close vs revisit; FW-0033 (scoring-formulation rework) promotion timing if per-component issues surface; FW-0036 (LAHC K-defaults) pin per-strategy module constants; FW-0035 (cloud-mode LAHC) becomes natural next step if verdict is positive. Pre-validation comparable-budget benchmark on the May 2026 ICU/HD dev-copy showed LAHC dominating SRB by ~+50 mean score points at equal wall-time budgets (paired t(4)=11.10, p≪0.001 across 5 seeds; SRB-K=100 ≈ LAHC-K=10 in wall time per FW-0036). M6 C1 closed 2026-05-07 per D-0067 (solver-strategy contract extension + LAHC algorithm spec). M6 C2 closed 2026-05-08 (LAHC implementation per §12A spec). M6 C3 closed 2026-05-08 (envelope-shape additive bump in `docs/selector_contract.md` §16.5 + CLI flags + pipeline / analyzer-chain integration tests; no `contractVersion` bumps; characterization surfaced FW-0036 for K-defaults). Cloud-mode LAHC integration parked as FW-0035 per D-0068. See `docs/delivery_plan.md` §7 / §9.
- **Closed-milestone trail:** D-0019..D-0054 (M1..M4); D-0055 activates M5; D-0056..D-0058 close M5 C1 (analyzer engine + analysis contract); D-0059 (apps_script directory rename); D-0060..D-0063 close M5 C2 (renderer + launcher route + cross-page nav; C3 dropped); D-0064 (forward-going Task 1/2/3 cadence vocabulary); D-0065 closes M5 (C4 live operator validation verdict); D-0066 activates M6 (LAHC-only scope); D-0067 closes M6 C1 (LAHC algorithm spec + solver_contract.md §12A; restates D-0066 sub-decision 10 — `contractVersion` bumps 1 → 2 driven by §9 promoting `strategyId` to a required boundary input; LAHC strategy registration alone via §11.2 would not have required a bump); D-0068 narrows M6 C3 to local-first (cloud-mode LAHC integration parked as FW-0035 per maintainer's local-first direction). Contracts settled through M6 C1 are listed in `docs/delivery_plan.md` §15.
