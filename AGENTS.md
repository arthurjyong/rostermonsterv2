...
- **Active checkpoint:** **M7 C3 — Async architecture decision (D-0071) + Codex P1.7 single-VM amendment** *(activated 2026-05-11 on M7 C2 closure; scope pivoted from sync UX to async architecture decision on 2026-05-11)*. M7 C2 closed 2026-05-11 — full Cloud Batch parallel-LAHC chain delivered with byte-identity exit criterion proven (5/5 audit tests). M7 C2 perftest revealed sync-budget brittleness driving the sync→async pivot; Codex P1.7 caught Cloud Batch's single-task-group restriction driving the single-VM pivot. Final M7 C4 architecture: Cloud Run thin front door + Cloud Batch single-task job (one `c3-highcpu-88` + Pool(88) + inline finalize after Pool.join) + launcher SECOND Web App deployment with `executeAs: USER_DEPLOYING` for the callback route + GCP ID-token auth in POST body + always-email-on-success-or-failure + concurrent-rejection by spreadsheet ID via Cloud Batch labels. M7 C3 is docs-only — D-0071 + Codex P1 fixes + supporting sweeps. Subsequent M7 C4 (async UX implementation per D-0071) / M7 C5 (live operator validation + M7 closure) per `docs/delivery_plan.md` §7.
